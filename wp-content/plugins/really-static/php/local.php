...
function rs_local_saveoptions(){
	global $rs_messsage;
	update_option ( 'rs_lokalerspeicherpfad', $_POST ['rs_lokalerspeicherpfad'] );
	if (substr ( $_POST ['rs_designremote'], - 1 ) != "/")$rs_messsage[e][]= __("You may forgot a / at the end of the path!", "reallystatic" );
...
function rs_local_deletefile($datei){
	unlink(get_option ( "rs_lokalerspeicherpfad" ).$datei);
