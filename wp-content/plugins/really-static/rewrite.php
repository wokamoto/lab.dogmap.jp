...
function home_urldddd($url, $path = "", $orig_scheme = "", $blog_id = "") {
	if (! really_static_selfdetect ())
		return $url;
		// S_LOG("url: $url");
		// S_LOG("A: ".loaddaten ( "rs_localurl", 'reallystatic' ));
		
	// S_LOG("B: ".loaddaten ( "rs_remoteurl", 'reallystatic' ));
		// S_LOG("C: ".get_option ( 'home' ) );
		
	// S_LOG("D: ".get_option ( 'siteurl' ));
		// S_LOG("E: ".str_replace( get_option ( 'home' )."/",loaddaten (
	// "rs_remoteurl", 'reallystatic' ),$url));
	if ((get_option ( 'home' ) . "/") != loaddaten ( "rs_remoteurl", 'reallystatic' ))
...
function really_static_urlfix1($url) {
	if (! really_static_selfdetect ())
		return str_replace ( loaddaten ( "rs_remoteurl", 'reallystatic' ), get_option ( 'home' ) . "/", $url );
	return $url;
