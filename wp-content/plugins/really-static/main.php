...
function RS_LOG($line, $typ = 0,$file=LOGFILE) {
	...
	@fwrite ( $fh, date ( "d M Y H:i:s",rs_getrealtime() ) . ": " . $line . "\r\n" );
	@fclose ( $fh );
}
function rs_getrealtime(){
	if(date ( "I")==0)$a=3600;
	else $a=0;
	return  time () + (get_option ( 'gmt_offset' ) * 3600 )-$a;
}
/**
 * @return void
 * @param String $d
...
function really_static_infotologfile($was, $info, $info2 = "") {
	...
	switch ($was) {
		...
		case ("cachehit uploadfile") :
			...
			break;
		case ("write file") :
			RS_log ( __ ( 'Writing File:', 'reallystatic' ) . " $info => $info2 @" . get_option ( "rs_lokalerspeicherpfad" ), 3 );
			break;
...
#return $file;
	...
	$file = str_replace ( loaddaten ( "rs_designlocal" ), loaddaten ( "rs_designremote" ), $file );
...
#erzeugte // am ende
	...
	if (loaddaten ( "rewritealllinked" ) == 1) {
		$file = str_replace ( loaddaten ( "rs_localurl", 'reallystatic' ), loaddaten ( "rs_remoteurl", 'reallystatic' ), $file );
		if (substr ( loaddaten ( "rs_localurl", 'reallystatic' ), - 1 ) == "/")
			$file = str_replace ( substr ( loaddaten ( "rs_localurl", 'reallystatic' ), 0, - 1 ), loaddaten ( "rs_remoteurl", 'reallystatic' ), $file );
		$file = str_replace ( urlencode ( loaddaten ( "rs_localurl", 'reallystatic' ) ), urlencode ( loaddaten ( "rs_remoteurl", 'reallystatic' ) ), $file );
 
		if (substr ( loaddaten ( "rs_localurl", 'reallystatic' ), - 1 ) == "/")
			$file = str_replace ( urlencode ( substr ( loaddaten ( "rs_localurl", 'reallystatic' ), 0, - 1 ) ), urlencode ( loaddaten ( "rs_remoteurl", 'reallystatic' ) ), $file );
	...
	#$file = preg_replace ( '#(Powered by)(\s+)<a(.*?)href=("|\')(.*?)("|\')(.*?)>WordPress MU</a>#si', '$1$2<a$3href=$4http://www.php-welt.net/really-static/$6$7>really static WordPress</a>', $file );
	$file = preg_replace ( '#<div id="site-generator">(.*?)<a href="http:(.*?)wordpress.org(.*?) rel="generator">(.*?)WordPress(.*?)</div>#sim', '<div id="site-generator"><a href="http://www.php-welt.net/really-static/">really static WordPress</a></div>', $file );
...
function really_static_bildurlrewrite($array) {
	...
	//RS_LOG("AA");
	if(strpos($array [3],loaddaten ( "rs_designlocal"))!==false)return "<img" . $array [1] . "src=" . $array [2] . loaddaten ( "rs_designremote" ).substr($array [3],strlen(loaddaten ( "rs_designlocal"))) . $array [4] . $array [5] . ">";
	//RS_LOG("BB");
	$url = $array [3];
...
function really_static_uploadfile($local, $remote) {
	...
	#RS_log ( __ ( 'Writing File:', 'reallystatic' ) . " $local => $remote @" . get_option ( "rs_lokalerspeicherpfad" ), 3 );
...
function reallystatic_configok($text, $typ = 1) {
	...
	elseif ($typ == 3) {
		...
		echo (str_pad('<script type="text/javascript">document.getElementById(\'refreshallinfog\').innerHTML = "'.date("H:i:s",rs_getrealtime()).'";  doingout("<b>Working on:</b> ' . $text . '");</script>',2048," "))."\n";#' . $text . "<br>";
	}
	@ob_flush ();
	flush ();
...
function wp_default_scripts2($scripts) {
	if (loaddaten ( "rs_donationid" ) == "-" and loaddaten ( "rs_counter" ) > 2000) {
	...
	if(get_option('rs_allrefreshcache')==array()){
		...
		foreach ( $lastposts as $post ) {
			...
			foreach ( loaddaten ( 'rs_posteditcreatedelete' ) as $v ) {
				...
				$ss = $wpdb->get_results ( $querystr, OBJECT );
				if ($ss [0]->datum > 0) {
					if(!$silent)  reallystatic_configok( __ ( "Skiping existing:", 'reallystatic' ) . " " . $t , 2 );
				} else {
