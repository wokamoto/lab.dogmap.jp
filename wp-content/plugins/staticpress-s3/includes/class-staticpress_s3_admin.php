...
class staticpress_s3_admin {
	...
	static public function option_keys(){
		return array(
			'access_key' => __('AWS Access Key',  staticpress_s3::TEXT_DOMAIN),
			'secret_key' => __('AWS Secret Key',  staticpress_s3::TEXT_DOMAIN),
			'region'     => __('AWS Region',  staticpress_s3::TEXT_DOMAIN),
			'bucket'     => __('S3 Bucket',  staticpress_s3::TEXT_DOMAIN),
			's3_url'     => __('S3 URL', staticpress_s3::TEXT_DOMAIN),
			);
	}
				...
				if (is_wp_error($options[$key])) {
					...
					foreach ($error_data->errors as $errors) {
						foreach ($errors as $error) {
							$err .= (!empty($err) ? '<br />' : '') . __('Error! : ', staticpress_s3::TEXT_DOMAIN);
							$err .= sprintf(
								__(str_replace($key, '%s', $error), staticpress_s3::TEXT_DOMAIN),
								$field
								);
						}
					}
			...
			if ($this->options !== $options) {
				...
				if ($this->options['s3_url'] !== $options['s3_url']) {
					...
					$sql = $wpdb->prepare(
						...
						staticpress_s3::META_KEY,
						staticpress_s3::META_KEY.'-replace'
						);
					$wpdb->query($sql);
				}
				printf(
					'<div id="message" class="updated fade"><p><strong>%s</strong></p></div>'."\n",
					empty($err_message) ? __('Done!', staticpress_s3::TEXT_DOMAIN) : $err_message
					);
	...
	public function options_page(){
		$option_keys   = $this->option_keys();
		$this->options = $this->get_option();
		$title = __('StaticPress S3 Option', staticpress_s3::TEXT_DOMAIN);
			...
			if ($regions && count($regions) > 0) {
				...
				foreach ($regions as $region) {
					$input_field .= sprintf(
						...
						__($region, staticpress_s3::TEXT_DOMAIN));
				}
