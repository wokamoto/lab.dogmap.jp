...
class staticpress_s3 {
	...
	function __construct($options){
		...
		add_action('StaticPress::file_put', array(&$this, 'file_put'), 10, 4);
	}
	...
	public function file_put($file_dest, $file_type, $file_date, $url){
	...
	// Upload file to S3
	private function s3_upload($filename, $S3_bucket, $S3_key, $force = true){
		if (!file_exists($filename))
			return false;

		$upload_result = false;
		if ($s3 = $this->s3($S3_bucket)) {
			if ($s3->object_exists($S3_key) && !$force)
				return true;
